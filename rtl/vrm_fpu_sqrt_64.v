...
/* ============================================================================
 * MODULE: vrm_fpu_sqrt_64 (FIXED V2 - Single Always Block)
 * DESCRIPTION: 
 * Fully Pipelined FP64 Square Root menggunakan Restoring Algorithm.
 * Latency: 57 cycles | Throughput: 1 result/cycle.
 * 
 * [FIXED V2]: Semua register di-drive dari SATU always block utama.
 *             Menggunakan generate untuk combinational logic saja.
 * ============================================================================ */
...
module vrm_fpu_sqrt_64 (
    ...
    // Eksponen Bias-adjusted
    wire [11:0] e_real = {1'b0, exp_a} - 12'd1023;
    wire exp_is_odd = e_real[0];

    // Siapkan radikan 110-bit
    wire [109:0] d_init = exp_is_odd ? {mant_a, 57'd0} : {1'b0, mant_a, 56'd0};
    ...
    localparam TOTAL_STAGES = STAGES + 1; // 0..56 = 57 stages
    
    // Deklarasi array pipeline (semua reg)
    ...
    // =========================================================================
    // 3. COMBINATIONAL NEXT-STATE LOGIC (Generate)
    // =========================================================================
    /*
     * Generate block hanya untuk menghitung NEXT STATE secara combinational.
     * Semua register di-drive oleh SATU always block di bawah.
     */
    
    // Wire arrays untuk next-state (combinational)
    ...
    generate
        for (i = 0; i < STAGES; i = i + 1) begin : SQRT_COMB
            // Tarik 2 bit teratas dari radikan stage sebelumnya
            wire [57:0] R_appended = {pipe_rem[i][55:0], pipe_rad[i][109:108]};
            
            // Test value: (root << 2) | 01
            wire [57:0] test_val = {pipe_root[i][55:0], 2'b01};
            
            // Pengurangan: R_appended - test_val
            wire [58:0] sub_res = {1'b0, R_appended} - {1'b0, test_val};
            wire can_sub = !sub_res[58];
            
            // Next values (combinational only, NO registers here!)
            assign next_rad[i]  = pipe_rad[i] << 2;
            assign next_rem[i]  = can_sub ? sub_res[57:0] : R_appended;
            assign next_root[i] = can_sub ? {pipe_root[i][55:0], 1'b1} : {pipe_root[i][55:0], 1'b0};
        end
    endgenerate

    // =========================================================================
    // 4. SINGLE ALWAYS BLOCK - All Pipeline Registers
    // =========================================================================
    /*
     * INI ADALAH SATU-SATUNYA BLOK ALWAYS YANG ME-DRIVE REGISTER!
     * Tidak ada multi-driven nets karena hanya satu driver per reg.
     */
    integer j;
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            // ============================================================
            // STAGES 1..56: Iterative SQRT (Shift register chain)
            // ============================================================
            // Manual unroll untuk menghindari generate loop di dalam always
            // (Verilog tidak support generate di dalam always)
    ...
    // =========================================================================
    // 5. POST-PROCESSING & PACKING (Stage 56)
    // =========================================================================
    wire [52:0] final_mant  = pipe_root[STAGES][55:3];
    wire        final_guard = pipe_root[STAGES][2];
    wire        final_round = pipe_root[STAGES][1];
    wire        sticky      = (pipe_rem[STAGES] != 0) || pipe_root[STAGES][0];
    ...
    assign result_out = pipe_inv[STAGES]  ? 64'h7FF8000000000000 : // NaN
                        pipe_inf[STAGES]  ? {pipe_sign[STAGES], 11'h7FF, 52'h0} : // +/- Infinity
                        pipe_zero[STAGES] ? {pipe_sign[STAGES], 63'd0} : // +/- Zero
                        final_fp;
